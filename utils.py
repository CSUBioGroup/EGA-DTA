    ...
    def __init__(self, root='/tmp', dataset='davis',
        ...
        if use_precomputed_esm2:
            ...
            if os.path.exists(file_5120):
                ...
                print(f'✓  ESM-2 15B  (5120): {target_file}')
            elif os.path.exists(file_2560):
                target_file = file_2560
                print(f'✓  ESM-2 3B  (2560): {target_file}')
            elif os.path.exists(file_1280):
                target_file = file_1280
                print(f'✓  ESM-2  (1280): {target_file}')
            
            if target_file:
                with open(target_file, 'rb') as f:
                    self.esm2_features = pickle.load(f)
                print(f'   {len(self.esm2_features)} ')
            else:
                print(f'⚠ Not found esm2_features_{base_dataset}_[5120|2560|1280].pkl')
                print(f'  Run precompute_ESM2_features.py')
